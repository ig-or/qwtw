...
#
#	foreach (item ${OUR_BOOST_COMPONENTS_LIST})
#		list(APPEND ${L_LIST} "Boost::${item}") 
#		string(TOUPPER ${item} iitem)
#		message(STATUS "Boost_${iitem}_LIBRARY_RELEASE = ${Boost_${iitem}_LIBRARY_RELEASE}")
#	message(STATUS "Boost_${iitem}_LIBRARY_DEBUG = ${Boost_${iitem}_LIBRARY_DEBUG}")
#	endforeach()	
...
macro (addQT)
	...
	list(APPEND ${L_LIST} Qt5::Core Qt5::Widgets Qt5::Gui) # Qt5::NetworkAuth Qt5::NetworkAuthPrivate)

	find_package(Qt5 COMPONENTS Network REQUIRED)
	list(APPEND ${L_LIST} Qt5::Network)

	find_package(Qt5 COMPONENTS OpenGL REQUIRED)
	list(APPEND ${L_LIST} Qt5::OpenGL)
	...
	if (NOT OUR_LIBRARY_DIR)
		...
		if (NOT OUR_LIBRARY_DIR)# lets create a new one; where?
			...
			IF (NOT EXISTS ${OUR_LIBRARY_DIR})
				get_filename_component(OUR_LIBRARY_DIR ${OUR_LIBRARY_DIR} ABSOLUTE BASE_DIR "${CMAKE_CURRENT_SOURCE_DIR}")
				if (NOT VCPKG_EXISTS)
					message(STATUS "creating ${OUR_LIBRARY_DIR} folder ")
					file(MAKE_DIRECTORY ${OUR_LIBRARY_DIR})
				else()
					message(STATUS "not creating ${OUR_LIBRARY_DIR} folder because VCPKG_EXISTS")
				endif()
			endif()
			IF (NOT EXISTS ${OUR_LIBRARY_DIR})
...
macro (addSourceFiles groupName sourceFilesPath)
	...
	get_filename_component(abs_file_path ${sourceFilesPath} ABSOLUTE BASE_DIR "${CMAKE_CURRENT_SOURCE_DIR}")
	#message(STATUS "source group ${ourGroupName} was added")
	set(extList "cpp" "cc" "c" "h" "hpp" "hh")
	set(sFileList)
	list(APPEND INC_DIR_LIST ${abs_file_path})
	foreach(sfName ${ARGN})
		set(currentFileList) # clear this list
		foreach(ext ${extList})
			set(aFile aFile-NOTFOUND)
			set(aFileName "${sfName}.${ext}")
			#message(STATUS "    looking for ${aFileName} in a ${abs_file_path}")
			find_file(aFile ${aFileName} PATHS ${abs_file_path} NO_DEFAULT_PATH)
			#message(STATUS "testing file ${aFileName} in ${abs_file_path}: ${aFile}")
			if(aFile) 
		...
		if (NOT currentFileList) # test without extencion
			find_file(aFile ${sfName} PATHS ${abs_file_path} NO_DEFAULT_PATH)
			if(aFile) 
				list(APPEND currentFileList ${aFile})
			endif()			
			set(aFile aFile-NOTFOUND)
		...
		if (NOT currentFileList) 
			message(STATUS "tested following files inside ${abs_file_path}: ")
			foreach(ext ${extList})
				message(STATUS "	${sfName}.${ext}")
			
			endforeach()
			message(FATAL_ERROR "error: in path ${abs_file_path} can not add file ${sfName}")
		else()
